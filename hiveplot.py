...
class HivePlot(object):
    ...
    def plot_nodes(self, nodelist, theta, group):
        for i, node in enumerate(nodelist):
            ...
            circle = plt.Circle(xy=(x,
                                    y),
                                radius=self.dot_radius,
                                color=self.node_colormap[group],
                                linewidth=0)
    ...
    def draw_edge(self, n1, n2, d, group):
        ...
        start_theta, end_theta = self.adjust_angles(
            n1, start_theta, n2, end_theta)
        ...
        verts = [
            (startx, starty), (middle1x, middle1y), (middle2x, middle2y), (endx, endy)]
    ...
    def adjust_angles(self, start_node, start_angle, end_node, end_angle):
        ...
        if start_group_idx == 0 and end_group_idx == len(
                self.nodes.keys()) - 1:
            if self.has_edge_within_group(start_group):
                start_angle = self.correct_negative_angle(
                    start_angle -
                    self.minor_angle)
            if self.has_edge_within_group(end_group):
                end_angle = self.correct_negative_angle(
                    end_angle +
                    self.minor_angle)
        ...
        # and (start_group_idx != 0 and end_group_idx !=
        # len(self.nodes.keys())-1):
        elif start_group_idx < end_group_idx:
            if self.has_edge_within_group(end_group):
                end_angle = self.correct_negative_angle(
                    end_angle -
                    self.minor_angle)
        ...
        # and (end_group_idx != 0 and start_group_idx !=
        # len(self.nodes.keys())-1):
